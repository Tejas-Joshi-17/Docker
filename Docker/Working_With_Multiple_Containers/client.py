...
def create_connection():
    return pymysql.connect(
        # host="localhost",           # While running simple python file use this
        host="172.17.0.2",            # IP address of container in which DB present
...
# 1) docker pull mysql
# 2) docker run -d --env MYSQL_ROOT_PASSWORD="password" --env MYSQL_DATABASE="userinfo" mysql:latest
# 3) docker inspect trusting_pasteur  (container_name in which mysql image running)